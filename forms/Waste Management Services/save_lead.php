<?php
// save_lead.php for Waste Management Services
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'waste_management_services_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($service_type)) { $errors[] = 'Field Service Required is required.'; }
    $property_type = isset($_POST['property_type']) ? trim($_POST['property_type']) : '';
    if (empty($property_type)) { $errors[] = 'Field Property Type is required.'; }
    $waste_volume = isset($_POST['waste_volume']) ? trim($_POST['waste_volume']) : '';
    if (empty($waste_volume)) { $errors[] = 'Field Approximate Waste Volume is required.'; }
    $collection_frequency = isset($_POST['collection_frequency']) ? trim($_POST['collection_frequency']) : '';
    if (empty($collection_frequency)) { $errors[] = 'Field Collection Frequency is required.'; }
    $site_address = isset($_POST['site_address']) ? trim($_POST['site_address']) : '';
    if (empty($site_address)) { $errors[] = 'Field Site Address is required.'; }

    if (empty($errors)) {
        // Prepare SQL insert statement
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `service_type`, `property_type`, `waste_volume`, `collection_frequency`, `site_address`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("sssssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $service_type, $property_type, $waste_volume, $collection_frequency, $site_address, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
