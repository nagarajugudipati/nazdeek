<?php
// save_lead.php for V Belt Dealers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'v_belt_dealers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($belt_type)) { $errors[] = 'Field V Belt Type is required.'; }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `belt_type`, `application`, `brand`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("sssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $belt_type, $application, $brand, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
