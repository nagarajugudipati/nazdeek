<?php
// save_lead.php for Weighing Machine Manufacturers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'weighing_machine_manufacturers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `machine_type`, `capacity`, `industry`, `custom_manufacturing`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $machine_type, $capacity, $industry, $custom_manufacturing, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
