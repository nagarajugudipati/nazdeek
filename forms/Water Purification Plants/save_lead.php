<?php
// save_lead.php for Water Purification Plants
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'water_purification_plants_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $plant_type = isset($_POST['plant_type']) ? trim($_POST['plant_type']) : '';
    if (empty($plant_type)) { $errors[] = 'Field Plant Type is required.'; }
    $capacity = isset($_POST['capacity']) ? trim($_POST['capacity']) : '';
    //if (empty($capacity)) { $errors[] = 'Field Capacity Required (LPH) is required.'; }
    $industry_type = isset($_POST['industry_type']) ? trim($_POST['industry_type']) : '';
    if (empty($industry_type)) { $errors[] = 'Field Industry Type is required.'; }
    $installation_address = isset($_POST['installation_address']) ? trim($_POST['installation_address']) : '';
    if (empty($installation_address)) { $errors[] = 'Field Installation Location is required.'; }

    if (empty($errors)) {
        // Prepare SQL insert statement
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `plant_type`, `capacity`, `industry_type`, `installation_address`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $plant_type, $capacity, $industry_type, $installation_address, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
