<?php
// save_lead.php for Paper Cup Making Machine Dealers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'paper_cup_making_machine_dealers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    //if (empty($capacity)) { $errors[] = 'Field Production Capacity Required is required.'; }
    $cup_size = isset($_POST['cup_size']) ? trim($_POST['cup_size']) : '';
    if (empty($cup_size)) { $errors[] = 'Field Cup Size Required is required.'; }
    $brand = isset($_POST['brand']) ? trim($_POST['brand']) : '';
    if (empty($brand)) { $errors[] = 'Field Preferred Brand (Optional) is required.'; }

    if (empty($errors)) {
        // Prepare SQL insert statement
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `machine_type`, `capacity`, `cup_size`, `brand`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $machine_type, $capacity, $cup_size, $brand, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
