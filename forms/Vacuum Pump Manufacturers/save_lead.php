<?php
// save_lead.php for Vacuum Pump Manufacturers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'vacuum_pump_manufacturers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($pump_type)) { $errors[] = 'Field Vacuum Pump Type is required.'; }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `pump_type`, `capacity`, `industry`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("sssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $pump_type, $capacity, $industry, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
