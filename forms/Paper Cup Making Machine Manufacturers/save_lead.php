<?php
// save_lead.php for Paper Cup Making Machine Manufacturers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'paper_cup_making_machine_manufacturers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($machine_type)) { $errors[] = 'Field Machine Type is required.'; }
    $capacity = isset($_POST['capacity']) ? trim($_POST['capacity']) : '';
    if (empty($capacity)) { $errors[] = 'Field Production Capacity is required.'; }
    $cup_size = isset($_POST['cup_size']) ? trim($_POST['cup_size']) : '';
    if (empty($cup_size)) { $errors[] = 'Field Cup Size Required is required.'; }
    $plant_location = isset($_POST['plant_location']) ? trim($_POST['plant_location']) : '';
    if (empty($plant_location)) { $errors[] = 'Field Installation Location is required.'; }

    if (empty($errors)) {
        // Prepare SQL insert statement
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `machine_type`, `capacity`, `cup_size`, `plant_location`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $machine_type, $capacity, $cup_size, $plant_location, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
