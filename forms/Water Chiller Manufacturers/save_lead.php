<?php
// save_lead.php for Water Chiller Manufacturers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'water_chiller_manufacturers_leads';
...
$sql_table = "CREATE TABLE IF NOT EXISTS `$table_name` (
    ...
    `chiller_type` VARCHAR(255) NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $chiller_type = isset($_POST['chiller_type']) ? trim($_POST['chiller_type']) : '';
    if (empty($chiller_type)) { $errors[] = 'Field Chiller Type is required.'; }
    $cooling_capacity = isset($_POST['cooling_capacity']) ? trim($_POST['cooling_capacity']) : '';
   // if (empty($cooling_capacity)) { $errors[] = 'Field Cooling Capacity Required is required.'; }
    $application = isset($_POST['application']) ? trim($_POST['application']) : '';
    if (empty($application)) { $errors[] = 'Field Application is required.'; }

    if (empty($errors)) {
        // Prepare SQL insert statement
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `chiller_type`, `cooling_capacity`, `application`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("sssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $chiller_type, $cooling_capacity, $application, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
