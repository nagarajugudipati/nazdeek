<?php
// save_lead.php for Welding Machine Dealers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'welding_machine_dealers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($machine_type)) { $errors[] = 'Field Welding Machine Type is required.'; }
    $power_capacity = isset($_POST['power_capacity']) ? trim($_POST['power_capacity']) : '';
   // if (empty($power_capacity)) { $errors[] = 'Field Power Capacity is required.'; }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `machine_type`, `power_capacity`, `application`, `brand`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
