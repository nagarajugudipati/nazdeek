<?php
// save_lead.php for Water Pump Dealers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'water_pump_dealers_leads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($pump_type)) { $errors[] = 'Field Pump Type is required.'; }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `pump_type`, `application`, `brand`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("sssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $pump_type, $application, $brand, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
