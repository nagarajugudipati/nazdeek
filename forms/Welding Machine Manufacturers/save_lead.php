<?php
// save_lead.php for Welding Machine Manufacturers
...
$db_pass = '';
$db_name = 'nazdeek';
$table_name = 'welding_machine_manufacturers_leads';
...
$sql_table = "CREATE TABLE IF NOT EXISTS `$table_name` (
    ...
    `power_capacity` VARCHAR(255) NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $power_capacity = isset($_POST['power_capacity']) ? trim($_POST['power_capacity']) : '';
    //if (empty($power_capacity)) { $errors[] = 'Field Power Capacity Required is required.'; }
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare("INSERT INTO `$table_name` (`name`, `mobile`, `email`, `city`, `pincode`, `locality`, `machine_type`, `power_capacity`, `industry`, `custom_manufacturing`, `quantity`, `requirements`, `preferred_time`, `budget`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("ssssssssssissd", $name, $mobile, $email, $city, $pincode, $locality, $machine_type, $power_capacity, $industry, $custom_manufacturing, $quantity, $requirements, $preferred_time, $budget);
            if ($stmt->execute()) {
